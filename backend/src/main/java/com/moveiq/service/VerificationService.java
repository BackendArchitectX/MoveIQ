...
public class VerificationService {
    private static final String METHODOLOGY = "verify-v2-idempotent-observation";
    ...
    public void onTrace(OperationTraceEvent trace) {
        ...
        if (!"EVENT_ACCEPTED".equals(trace.eventType()) || trace.scopeKey() == null || trace.sourceEventId() == null) return;
        ...
        observe(trace.sourceEventId(), scope[0], nullableScope(scope[1]), nullableScope(scope[2]), nullableScope(scope[3]),
    ...
    private void observe(String sourceEventId, String businessUnit, String office, String shift, String direction,
                         java.time.Instant eventTime, double delay) {
        jdbc.update("""
                WITH eligible AS (
                    SELECT execution_id
                    FROM moveiq.verification_snapshot
                    WHERE business_unit = ?
                      AND office IS NOT DISTINCT FROM ?
                      AND shift IS NOT DISTINCT FROM ?
                      AND direction IS NOT DISTINCT FROM ?
                      AND baseline_event_time < ?
                ), inserted AS (
                    INSERT INTO moveiq.verification_observation(execution_id, source_event_id, event_time, delay_minutes)
                    SELECT execution_id, ?, ?, ? FROM eligible
                    ON CONFLICT (execution_id, source_event_id) DO NOTHING
                    RETURNING execution_id
                )
                UPDATE moveiq.verification_snapshot v
                SET observed_sample_size = v.observed_sample_size + 1,
                    observed_delay_sum = v.observed_delay_sum + ?,
                    outcome = CASE
                        WHEN v.observed_sample_size + 1 < ? THEN 'INSUFFICIENT_EVIDENCE'
                        WHEN v.baseline_avg_delay <= 0 THEN 'INSUFFICIENT_EVIDENCE'
                        WHEN (((v.observed_delay_sum + ?) / (v.observed_sample_size + 1)) - v.baseline_avg_delay)
                             / v.baseline_avg_delay * 100 <= -? THEN 'OBSERVED_IMPROVEMENT'
                        WHEN (((v.observed_delay_sum + ?) / (v.observed_sample_size + 1)) - v.baseline_avg_delay)
                             / v.baseline_avg_delay * 100 >= ? THEN 'WORSENED'
                        ELSE 'NO_MATERIAL_CHANGE'
                    END,
                    updated_at = now()
                FROM inserted i
                WHERE v.execution_id = i.execution_id
                """,
                businessUnit, office, shift, direction, Timestamp.from(eventTime),
                sourceEventId, Timestamp.from(eventTime), delay,
