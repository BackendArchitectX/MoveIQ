...
public class OperationTraceService {
    ...
    public OperationTraceEvent record(
        ...
        String evidenceJson;
        try {
            evidenceJson = objectMapper.writeValueAsString(safeEvidence);
        } catch (JsonProcessingException e) {
            throw new IllegalStateException("Unable to serialize trace evidence", e);
        }

        OperationTraceEvent trace = jdbc.queryForObject(
                """
                ...
                """,
                (rs, rowNum) -> new OperationTraceEvent(
                        rs.getLong("sequence"),
                        sessionId,
                        situationId,
                        sourceEventId,
